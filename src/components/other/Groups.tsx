...
interface GroupsProps {
	activeGroup: string;
	groups: string[];
	onGroupNamePressed: (group: string, index: number) => void;
...
const Groups = ({ groups, activeGroup, onGroupNamePressed, onAddGroupPressed, onDeleteGroupPressed }: GroupsProps) => {
	...
	const handleOnPress = (group: string, idx: number) => {
					...
					{groups.map((group, index) => (
						<RippleButton
							{...(isDeleteMode[group] ? {
								...
								onPress: () => handleOnPress(group, index),
							})}
